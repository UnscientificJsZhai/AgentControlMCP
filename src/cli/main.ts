...
import { createMcpTools, describeTool, toolsetSchema } from '../transport/mcp/catalog.js';
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (group === 'bridge') {
    await startBridge(z.string().min(1).parse(options.binding));
    return;
  }
  let paths = resolveStoragePaths({ dataDir });
  if (group === 'serve') {
    const toolset = toolsetSchema.parse(options.toolset ?? 'collaboration');
  ...
  if (group === 'tools') {
    output(
      options.mcp
        ? createMcpTools(
            {} as Container,
            toolsetSchema.parse(options.toolset ?? 'collaboration'),
          ).map(describeTool)
        : [...createTools({} as Container), ...createCollaborationTools({} as Container)].map(
            (tool) => ({
              name: tool.name,
  ...
  const collaboration = createCollaborationTools({} as Container);
  const definition = [...createTools({} as Container), ...collaboration].find(
    (item) => item.name === name,
  );
  if (collaboration.some((tool) => tool.name === name) && !definition?.readOnly && !args.requestId)
    args.requestId = id('cli');
  if (
    definition &&
  ...
  if (
    ...
    (/^(runtime_|session_create|session_load|session_resume|agent_authenticate|task_submit)/.test(
      ...
      collaboration.some((tool) => tool.name === name))
  )
    fail(
